...
static const uint8_t tempChar[]   = "Temprature Value";
static bool temp_notifications_enabled = false;

static void on_connect(ble_th_svc_t * p_simple_service, ble_evt_t const * p_ble_evt)
{
    p_simple_service->conn_handle = p_ble_evt->evt.gap_evt.conn_handle;
    temp_notifications_enabled = false;
}

static void on_disconnect(ble_th_svc_t * p_simple_service, ble_evt_t const * p_ble_evt)
{
...
static void on_write(ble_th_svc_t * p_simple_service, ble_evt_t const * p_ble_evt)
{
    ble_gatts_evt_write_t const * p_evt_write = &p_ble_evt->evt.gatts_evt.params.write;

    // Custom Value Characteristic Written to.
    if (    (p_evt_write->handle == p_simple_service->temp_char_handles.cccd_handle)
        &&  (p_evt_write->len == 2))
    {
        ble_th_evt_t evt;
        ...
        {
            NRF_LOG_INFO("Notifications ENABLED for temp");
            evt.evt_type = BLE_TEMP_EVT_NOTIFICATION_ENABLED;
            temp_notifications_enabled = true;
        }
        else
        {
            NRF_LOG_INFO("Notifications DISABLED for temp");
...
static uint32_t temp_char_add(ble_th_svc_t * p_simple_service)
{
    ...
    ble_gatts_char_pf_t form;
    form.format = BLE_GATT_CPF_FORMAT_FLOAT32; //setting up value format to float

    // Characteristic Metadata
    char_md.char_props.read          = 1;
    char_md.char_props.notify        = 1;
    char_md.p_char_user_desc         = tempChar;
    char_md.char_user_desc_size      = sizeof(tempChar);
    char_md.char_user_desc_max_size  = sizeof(tempChar);
    char_md.p_char_pf                = &form;
    ...
    ble_uuid.uuid = BLE_UUID_TEMP_INFO_CHAR_UUID;
    ...
    return sd_ble_gatts_characteristic_add(p_simple_service->service_handle, &char_md,
                                           &attr_char_value,
                                           &p_simple_service->temp_char_handles);
}
...
uint32_t ble_th_svc_init(ble_th_svc_t * p_simple_service, ble_th_evt_handler_t app_evt_handler)
{
    ...
    // Add service UUID
    ble_uuid128_t base_uuid = {BLE_UUID_TH_SVC_BASE_UUID};
    ...
    ble_uuid.type = p_simple_service->uuid_type;
    ble_uuid.uuid = BLE_UUID_TH_SVC_UUID;
    ...
    err_code = temp_char_add(p_simple_service);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
...
{
    ble_th_svc_t * p_simple_service = (ble_th_svc_t *)p_context;
...
void temp_characteristic_update(ble_th_svc_t * p_simple_service, float *button_action)
{
    ...
    if (p_simple_service->conn_handle != BLE_CONN_HANDLE_INVALID)
    {
        ...
        gatts_value.p_value = (uint8_t*)button_action;

        // Update database.
        err_code = sd_ble_gatts_value_set(p_simple_service->conn_handle,
                                          p_simple_service->temp_char_handles.value_handle,
                                          &gatts_value);
        APP_ERROR_CHECK(err_code);

        if (temp_notifications_enabled)
        {
            NRF_LOG_INFO("Sending notification for temperature value");
            uint16_t               len = sizeof (float);
            ble_gatts_hvx_params_t hvx_params;
            memset(&hvx_params, 0, sizeof(hvx_params));

            hvx_params.handle = p_simple_service->temp_char_handles.value_handle;
